...
function transformToLis(obj) {
  let arrayOfElem = [];
  for (let key in obj) {
    arrayOfElem.push(`<li>${key}: ${obj[key]}</li>`);
  }
  return arrayOfElem;
}
...
// you have an array of arrays and when it is given an integer as a parameter, it will count the total amount of times this integer is present in array of arrays
...
// lol as a stupid 1 liner :'D
// const divisibleByFiveTwoToThePower = (input) => input.map(row => row.filter(cell => typeof cell === 'number' && cell % 5 === 0).map(cell => Math.pow(2, cell)));
